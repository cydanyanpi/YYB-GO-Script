...
YYB-GO 适配版：从环境变量 YYB_SERVER 读取服务地址，动态获取微信 code 换 token
...
def get_code(server_entry):
    ...
    try:
        ...
        print("[code服务] %s 获取 code 异常: %s" % (server, exc))
        return None
    code = (((data.get("data") or {}).get("result") or {}).get("code"))
    if data.get("code") == 0 and code:
        print("[code服务] %s code 获取成功" % server)
        return code
    print("[code服务] %s code 获取失败: %s" % (server, json.dumps(data, ensure_ascii=False)[:400]))
    return None
...
def _run_single_account(args, account_label="账号1/1", headers=None):
    ...
    if headers is None:
        raise RuntimeError(
            ...
            "请检查 YYB_SERVER 配置与 YYB Go 服务是否可用。")
