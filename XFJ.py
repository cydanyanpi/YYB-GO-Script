...
TOKEN_CACHE_FILE = os.path.join(SCRIPT_DIR, "token_caches", "xfj_token_cache.json")
...
def main():
    ...
    for i, entry in enumerate(SERVERS):
        ...
        if access_token:
            print(f"  [\u7f13\u5b58] \u4f7f\u7528\u7f13\u5b58 access_token: {access_token[:15]}...")
            print(f"\u2713 \u8d26\u53f7[{i+1}] ACCESS_TOKEN={access_token}")
            if i < len(SERVERS) - 1:
                time.sleep(3)
            continue
        ...
        if result:
            ...
            cache = read_token_cache()
            cache[ref] = {"access_token": access_token}
            write_token_cache(cache)
            print(f"  [\u7f13\u5b58] access_token \u5df2\u5199\u5165\u7f13\u5b58")
            print(f"\u2713 \u8d26\u53f7[{i+1}] ACCESS_TOKEN={access_token}")
        else:
            print(f"\u2717 \u8d26\u53f7[{i+1}] \u767b\u5f55\u5931\u8d25")
